...
int main() {
    ...
    highestPaidPlayer(barcelona_player, SIZE);
    return 0;
}
...
void highestPaidPlayer(struct BarcelonaPlayer *pl, int size) {
    struct BarcelonaPlayer *max_ptr = pl;
    for(struct BarcelonaPlayer *ptr = pl + 1; ptr < pl + size; ++ptr)
        if(ptr -> Salary > max_ptr -> Salary)
            max_ptr = ptr;
    writeInfo(max_ptr, "The highest-paid player's information:\n");
}
