...
int partial_search(struct Student student[]);
...
int main() {
    ...
    while(1) {
        ...
        switch(choice) {
            ...
            case 4: partial_search(student); break;
            case 5: edit(student); break;
            case 6: delete_index(student); break;
...
int partial_search(struct Student student[]) {  // Similar to strstr() implementation from <string.h>
    int found_count = 0;
    if(!count_student) {
        printf("Not found.\n");
        return found_count;
    ...
    char search_key[SIZE];
    printf("Enter student's name to search partially: ");
    fflush(stdin);
    gets(search_key);
    for(int i = 0, haystack = 0, needle; i < count_student; i++, haystack = 0)
        while(student[i].name[haystack] != '\0') {
            needle = 0;
            int haystack_copy = haystack;
            while(search_key[needle] != '\0' && student[i].name[haystack_copy] != '\0' && search_key[needle] == student[i].name[haystack_copy]) {
                ++needle;
                ++haystack_copy;
            }
            if(search_key[needle] == '\0') {
                ++found_count;
                printf("Index = %d\n", i);
                view_indexed(student[i]);
            }
