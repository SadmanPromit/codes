...
class BILSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim, dropout_prob=DROPOUT_RATE):
        ...
        self.fc1 = nn.Linear(hidden_dim * 2, 128)  # times 2 because of bidirectionality
...
def train_model(train_loader, val_loader, model, criterion, optimizer, scheduler, n_epochs=EPOCHS):
    ...
    df.to_excel('./output/log_bilstm.xlsx', index=False)
...
def test_model(epoch, model, test_loader):
    ...
    with open(f'./output/pm_bilstm.txt', 'w') as f:
    ...
    plt.savefig(f'./output/cm_bilstm.png', dpi=600)
    plt.close()
    torch.save(model.state_dict(), f'./output/st_bilstm.pth')
    return accuracy
...
if __name__ == "__main__":
    ...
    input_dim = 13  # 13 features based on your previous dataset
    hidden_dim = 64
    num_layers = 1
    output_dim = 4  # 4 classes for prediction

    model = BILSTM(input_dim, hidden_dim, num_layers, output_dim).cuda()
