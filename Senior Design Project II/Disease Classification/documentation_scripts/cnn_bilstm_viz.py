...
class CNNBILSTM(nn.Module):
    def __init__(self, dropout_prob=DROPOUT_RATE):
        ...
        self.lstm = nn.LSTM(64, 64, bidirectional=True, batch_first=True)  # BiLSTM layer
        self.fc1 = nn.Linear(128 * 13, 128)  # Input size changed to account for BiLSTM output
    ...
    def forward(self, x):
        ...
        # BiLSTM
        lstm_out, _ = self.lstm(out_conv2.transpose(1, 2))  # Transpose to have sequence length in the second dimension
        lstm_out_flat = lstm_out.contiguous().view(lstm_out.size(0), -1)  # Flatten LSTM outputs to feed into FC layer
...
def train_model(train_loader, val_loader, model, criterion, optimizer, scheduler, n_epochs=EPOCHS):
    ...
    df.to_excel('./output/log_cnnbilstm.xlsx', index=False)
...
def test_model(epoch, model, test_loader):
    ...
    with open(f'./output/pm_cnnbilstm.txt', 'w') as f:
    ...
    plt.savefig(f'./output/cm_cnnbilstm.png', dpi=600)
    plt.close()
    torch.save(model.state_dict(), f'./output/st_cnnbilstm.pth')
    return accuracy
...
if __name__ == "__main__":
    ...
    dot.format = 'png'
    dot.attr(dpi='600')
    dot.render("cnn_bilstm_architecture")