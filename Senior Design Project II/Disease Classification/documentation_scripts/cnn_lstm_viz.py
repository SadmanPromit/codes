...
class CNNLSTM(nn.Module):
    def __init__(self, dropout_prob=DROPOUT_RATE):
        ...
        # Introduce LSTM layers
        self.lstm = nn.LSTM(64 * 13, 64, batch_first=True)
    ...
    def forward(self, x):
        ...
        out_conv2_flat = out_conv2.view(out_conv2.size(0), -1, 64 * 13)  # reshaping for LSTM

        # Passing through LSTM layers
        lstm_out, _ = self.lstm(out_conv2_flat)
        lstm_out = lstm_out[:, -1, :]  # Taking the last output of sequence for classification

        out_fc1 = self.dropout(nn.LeakyReLU()(self.fc1(lstm_out)))
...
def train_model(train_loader, val_loader, model, criterion, optimizer, scheduler, n_epochs=EPOCHS):
    ...
    df.to_excel('./output/log_cnnlstm.xlsx', index=False)
...
def test_model(epoch, model, test_loader):
    ...
    with open(f'./output/pm_cnnlstm.txt', 'w') as f:
    ...
    plt.savefig(f'./output/cm_cnnlstm.png', dpi=600)
    plt.close()
    torch.save(model.state_dict(), f'./output/st_cnnlstm.pth')
    return accuracy
...
if __name__ == "__main__":
    ...
    dot.format = 'png'
    dot.attr(dpi='600')
    dot.render("cnn_lstm_architecture")